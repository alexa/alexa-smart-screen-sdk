...
DEVICE_SDK_CLONE_TAG=${DEVICE_SDK_CLONE_TAG:-'v1.17.0'}
SMART_SCREEN_SDK_CLONE_URL=${SMART_SCREEN_SDK_CLONE_URL:-'https://github.com/alexa/alexa-smart-screen-sdk.git'}
...
# Default device configuration flags
DEVICE_SDK_CONFIG_MANUFACTURER_NAME=${DEVICE_SDK_CONFIG_MANUFACTURER_NAME:-"Test-Manufacturer"}
DEVICE_SDK_CONFIG_DEVICE_DESCRIPTION=${DEVICE_SDK_CONFIG_DEVICE_DESCRIPTION:-"Test-Device"}
...
function genConfig {
	...
	-DSDK_CONFIG_MANUFACTURER_NAME="${DEVICE_SDK_CONFIG_MANUFACTURER_NAME}" \
	-DSDK_CONFIG_DEVICE_DESCRIPTION="${DEVICE_SDK_CONFIG_DEVICE_DESCRIPTION}"
}
...
function show_help {
    ...
    echo  '  -m <device-manufacturer>  	If nothing is provided, the default device manufacturer is "Test Manufacturer"'
    echo  '  -d <device-description>	If nothing is provided, the default device description is "Test Device"'
...
function buildAplCoreEngine {
    pushd ${APL_CORE_ENGINE_BUILD_PATH}
        ...
        then
            ...
            cmake -DCMAKE_POSITION_INDEPENDENT_CODE=ON $APL_CORE_ENGINE_SOURCE_PATH
        fi
       make
    popd
}
...
while getopts "$OPTIONS" opt ; do
    case $opt in
        s )
            ...
            ;;
        d )
            DEVICE_SDK_CONFIG_DEVICE_DESCRIPTION="$OPTARG"
            ;;
        m )
            DEVICE_SDK_CONFIG_MANUFACTURER_NAME="$OPTARG"
            ;;
        h )
            show_help
            exit 1
