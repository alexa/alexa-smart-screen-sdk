...
public:
     ...
     */
    virtual void onActivityStarted(const std::string& source) = 0;

    /**
     * The given activity has ended
     * @param source The activity type
     */
    virtual void onActivityEnded(const std::string& source) = 0;

    /**
     * An APL send event command was executed
     * @param event The event
     */
    virtual void onSendEvent(const std::string& event) = 0;
     ...
     */
    virtual void onVisualContextAvailable(unsigned int stateRequestToken, const std::string& context) = 0;

    /**
     * Called when the document idle timeout is set
     * @param timeout The timeout value
     */
    virtual void onSetDocumentIdleTimeout(const std::chrono::milliseconds& timeout) = 0;
     ...
     */
    virtual void onDataSourceFetchRequestEvent(const std::string& type, const std::string& payload) = 0;

    /**
     * Handles a RuntimeError event
     * @param payload The payload for the error event
     */
    virtual void onRuntimeErrorEvent(const std::string& payload) = 0;
